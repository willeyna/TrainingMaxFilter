...
n_epochs = 5 # The number of training epochs for each model
    ...
    for epoch in range(n_epochs):
        ...
        with torch.no_grad():
            ...
            for i in range(0, n, block_size):
                ...
                fXi = L @ shape_max_filter(norm_templates, X_test[:, i:i+block_size])
                for j in range(i, n, block_size):
                    Xj = X_test[:, j:j+block_size]
                    fXj = L @ shape_max_filter(norm_templates, X_test[:, j:j+block_size])
                    # compute block distance matrices
                    # when Xi=Xj function automatically only computes n choose 2 distances
                    DX_ij   = shape_dist_matrix(Xi, Xj)
